...
export function PairingScreen({ codigo, painelUrl }: Props) {
  ...
  return (
    <div className="absolute inset-0 bg-black text-white grid place-items-center px-8 overflow-hidden">
      {/* Glow background */}
      ...
      <div className="relative z-10 max-w-4xl w-full grid grid-cols-1 md:grid-cols-[1fr_auto] items-center gap-10 md:gap-16">
        <div>
          ...
          <h1 className="mt-10 text-2xl md:text-3xl font-medium text-white/80">
            Para ativar esta TV, digite o código abaixo no painel administrativo:
          </h1>

          <div className="mt-8 flex items-center gap-4 md:gap-6">
            {grupos.map((g, gi) => (
              <div key={gi} className="flex gap-2 md:gap-3">
                {g.split("").map((d, di) => (
                  <div
                    key={di}
                    className="h-24 w-20 md:h-32 md:w-28 rounded-2xl border border-primary/40 bg-primary/10 grid place-items-center font-mono text-5xl md:text-7xl font-bold text-primary shadow-[0_0_40px_-10px_rgba(82,130,255,0.6)]"
                  >
                    {d}
                  </div>
                ))}
        ...
        <div className="flex flex-col items-center gap-3 bg-white/5 border border-white/10 rounded-2xl p-5 backdrop-blur">
          <div className="rounded-xl bg-white p-3">
            <QRCodeSVG value={painelUrl} size={140} />
          </div>
          <p className="text-xs text-white/60 text-center max-w-[160px]">
            Ou escaneie para abrir o painel
          </p>
        </div>
      </div>
    </div>
